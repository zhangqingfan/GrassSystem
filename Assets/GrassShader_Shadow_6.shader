Shader "MyShader/GrassShader_Shadow_6"
{
    Properties
    {
    ...
    geometryVertex CreateVertex(float3 pos, float2 uv, float normal)
    {
        geometryVertex o;
    ...
    void geo(triangle vertexOutput input[3] : SV_POSITION, inout TriangleStream<geometryVertex> newTriangle)
    {
        ...
        for(int i = 0; i < GRASS_SEGMENTS; i++)
        {
            ...
            geometryVertex o = CreateVertex(pos + mul(tranformMatrix, float3(width, curve, height)), float2(0, t), mul(tranformMatrix, tangentNormal));
            newTriangle.Append(o);

            o = CreateVertex(pos + mul(tranformMatrix, float3(-width, curve, height)), float2(0, t), mul(tranformMatrix, tangentNormal));
            newTriangle.Append(o);
        }

        float3 tangentNormal = float3(0, -1, 0); //TODO
        geometryVertex o = CreateVertex(pos + mul(tranformMatrix, float3(0, _Curve, _Height)), float2(0, 1), mul(tranformMatrix, tangentNormal)); //in TBN space the z is y in model space
        newTriangle.Append(o);
    }
    ENDCG
